...
public class RepositorioContaMilhagem {
	...
	public void incluir(ContaMilhagem novo) { // Metodo para incluir conta
		if (qtd < conta.length) { // Verifica se ha espaco no array
			if ((buscar(novo.getCliente().getCpf(), novo.getIdentificadorConta())) == null) { // Verificacao
																								// do
																								// cpf
																								// e
																								// IdentificadorConta
	...
	public ContaMilhagem buscar(Cpf cpf, IdentificadorConta id) { // Metodo para
																	// buscar
																	// conta
																	// pelo cpf
																	// e
		...
		for (int i = 0; i < qtd; i++) { // Loop para encontrar a conta
			if (cpf.getCpf().equals(conta[i].getCliente().getCpf().getCpf())
					&& (id.getIdentificadorConta() == (conta[i].getIdentificadorConta().getIdentificadorConta()))) { // Verificacao
																														// da
																														// conta
																														// pelo
																														// cpf
	...
	public boolean excluir(Cpf cpf, IdentificadorConta id) { // Metodo para
																// excluir conta
		...
		for (int i = 0; i < qtd; i++) { // Loop para encontrar a conta
			if (cpf.getCpf().equals(conta[i].getCliente().getCpf().getCpf())
					&& (id.getIdentificadorConta() == (conta[i].getIdentificadorConta().getIdentificadorConta()))) { // Verificacao
																														...
																														// e
																														// identificadorConta
				conta[i] = conta[qtd]; // Atribui a ultima conta a conta
										// encontrada
	...
	public boolean alterar(ContaMilhagem antigo, ContaMilhagem novo) { // Metodo
																		// para
																		// alterar
																		// informacoes
																		// da
																		// conta
		if (this.excluir(antigo.getCliente().getCpf(), antigo.getIdentificadorConta())) { // Caso
																							// exista
																							// uma
																							// conta
																							// e
	...
	public void buscarTodos() { // Metodo para listar todas as informacoes do
								// array
